...
**Changelog 条目格式要求**:
每个迭代章节必须包含以下三部分内容：
1. **主要改动**: 本迭代新增/修改的类、接口、方法
2. **知识点**: 本迭代涉及的核心概念和设计模式
3. **关键单元测试**: 本迭代添加的测试类和测试方法
...
3. **Given** 任意迭代章节, **When** 查看内容, **Then** 包含主要改动、知识点和关键单元测试三部分
...
- 迁移示例代码时，如果测试包中已存在同名文件怎么办？→ 覆盖测试包中的同名文件
- 合并分支时出现冲突怎么办？→ 优先保留旧分支（前一个分支）的改动
...
- **SC-008**: 每次处理分支后，必须验证 `mvn compile` 和 `mvn test` 都通过
...
- Q: changelog.md 每个迭代章节应包含哪些内容？ → A: 主要改动、知识点、关键单元测试
- Q: 分支合并冲突时优先保留哪个分支的改动？ → A: 优先保留旧分支（前一个分支）的改动
- Q: 迁移示例代码时，测试包中已存在同名文件如何处理？ → A: 覆盖测试包中的同名文件
- Q: 每次处理分支时需要满足什么验证条件？ → A: 所有代码编译通过且所有单元测试通过
